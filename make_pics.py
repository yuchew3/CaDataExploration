...
# Each function generates some kind of visualization given a result file.
# See the argument to np.load() to find out what kind of result it takes.

def autocorr():
    names = ["Nearest Neighbors", "Linear SVM", "RBF SVM", "Gaussian Process",
         "Decision Tree", "Random Forest", "Neural Net", "AdaBoost",
         "Naive Bayes", "QDA", "XGBoost"]
    test_accu = np.zeros((5, len(names)))
    for i in range(6,11):
        ind = i - 6
        test_accu[ind] = np.load('../data/clf_results/autocorr_len'+str(2*i)+'_lag'+str(i)+'.npy')
    lags = range(6,11)
    figure = plt.figure()
    ax = plt.subplot(111)
    for i in range(11):
        ax.plot(lags, test_accu[:,i], label=names[i])
    box = ax.get_position()
